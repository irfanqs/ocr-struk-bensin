...
def fill_rekap(rows: list[dict[str, Any]], template_path: Path, output_path: Path) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        if not trx_date:
            logs.append(fail(log, "tanggal_tidak_ditemukan"))
            continue
        if not plate:
            logs.append(fail(log, "nopol_tidak_ditemukan"))
            continue
        if not fuel:
            logs.append(fail(log, "jenis_bbm_tidak_dikenali"))
            continue

        sheet_name = f"{MONTHS[trx_date.month]} {trx_date.year}"
        if sheet_name not in wb.sheetnames:
            logs.append(fail(log, f"sheet_tidak_ada:{sheet_name}"))
            continue

        ws = wb[sheet_name]
        if sheet_name not in column_cache:
            column_cache[sheet_name] = build_column_pairs(ws)

        pair = find_column_pair(column_cache[sheet_name], plate, fuel)
        if not pair:
            logs.append(fail(log, "kolom_nopol_bbm_tidak_ditemukan"))
            continue

        row_range = find_date_rows(ws, trx_date)
        if not row_range:
            logs.append(fail(log, "baris_tanggal_tidak_ditemukan"))
            continue

        target_row, fill_status = find_target_row(ws, row_range, pair, volume, amount)

        if fill_status == "already_filled":
            existing_liter = ws.cell(target_row, pair.liter_col).value
            existing_amount = ws.cell(target_row, pair.amount_col).value
            log.update(
                {
                    "status": "already_filled",
                    "reason": f"sudah terisi: {existing_liter} L / Rp {existing_amount}",
                    "sheet": sheet_name,
                    "excel_row": target_row,
                    "liter_col": pair.liter_col,
                    "rupiah_col": pair.amount_col,
                }
            )
            logs.append(log)
            continue
        ...
        if target_row is None:
            logs.append(fail(log, "baris_tanggal_penuh"))
            continue

        ws.cell(target_row, pair.liter_col).value = volume
        ws.cell(target_row, pair.amount_col).value = amount
        copy_style_from_above(ws, target_row, pair.liter_col)
        copy_style_from_above(ws, target_row, pair.amount_col)
...
def build_column_pairs(ws: Any) -> list[ColumnPair]:
    pairs: list[ColumnPair] = []
    col = 2
    while col < ws.max_column:
        current = cell_value(ws, 6, col)
        next_value = cell_value(ws, 6, col + 1)
        if not current or normalize_text(next_value) != "rp":
            col += 1
            continue

        current_norm = normalize_text(current)
        header_current = cell_value(ws, 5, col)
        header_next = cell_value(ws, 5, col + 1)
        fuel = normalize_fuel(header_current if current_norm.startswith("liter") else current)
        plates = extract_plates(" ".join(str(v or "") for v in (header_current, header_next)))
...
def find_column_pair(pairs: list[ColumnPair], plate: str, fuel: str) -> ColumnPair | None:
    exact = [pair for pair in pairs if plate in pair.plates and pair.fuel == fuel]
    if exact:
        return exact[0]

    # Some receipts say Bio Solar while the sheet only says Solar.
    for pair in pairs:
        if plate in pair.plates and pair.fuel and fuel in {pair.fuel, normalize_fuel(pair.fuel)}:
            return pair
    return None


def find_date_rows(ws: Any, target_date: date) -> tuple[int, int] | None:
    for row in range(1, ws.max_row + 1):
        value = cell_value(ws, row, 1)
...
def find_target_row(
    ...
    """Returns (row, status). Status: 'empty' (bisa diisi), 'already_filled' (duplikat), 'full' (semua baris penuh)."""
    empty_row = None
...
def copy_style_from_above(ws: Any, row: int, col: int) -> None:
    if row <= 1:
        return
...
def write_log_sheet(wb: Any, logs: list[dict[str, Any]]) -> None:
    ...
    ws.append(columns)
    for log in logs:
        ws.append([log.get(column) for column in columns])
    ws.freeze_panes = "A2"
    ws.auto_filter.ref = ws.dimensions


def write_raw_sheet(wb: Any, rows: list[dict[str, Any]]) -> None:
...
def write_review_sheet(wb: Any, logs: list[dict[str, Any]]) -> None:
    ...
    columns = ["status", "keterangan", "tanggal", "nopol", "produk", "volume_liter", "total_rupiah", "sheet", "excel_row", "source_pdf", "page"]
    ws.append(columns)
    for cell in ws[1]:
    ...
    for log in flagged:
        ...
        reason = log.get("reason") or _STATUS_LABELS.get(log.get("reason", ""), log.get("reason", ""))
        if status == "skipped":
...
def fail(log: dict[str, Any], reason: str) -> dict[str, Any]:
    log["status"] = "skipped"
    log["reason"] = reason
    return log
...
def apply_source_period(value: date | None, source_pdf: Any) -> date | None:
    source_period = infer_period_from_source(source_pdf)
...
def infer_period_from_source(source_pdf: Any) -> tuple[int, int] | None:
    if source_pdf is None:
        return None
    text = normalize_text(source_pdf).upper()
...
def extract_plates(text: str) -> set[str]:
    plates = set()
