...
def make_rotation_variants(image_path: Path, output_dir: Path, stem: str, rotations: list[int] | None = None) -> list[Path]:
    ...
    with Image.open(image_path) as original:
        ...
        for rotation in rotations:
            image = base.rotate(rotation, expand=True)
            image.thumbnail((1800, 1800))
            out = output_dir / f"{stem}_rot{rotation}.jpg"
            image.convert("RGB").save(out, "JPEG", quality=78, optimize=True)
            paths.append(out)
